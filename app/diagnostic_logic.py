...
def build_parent_message(
    ...
    return (
        "Здравствуйте!\n\n"
        "### Итог диагностики\n"
        f"- **Работа:** «{work.exam_type}».\n"
        f"- **Результат:** {total_score}/{work.max_score}.\n"
        f"- **Текущий уровень:** {band['level']}.\n\n"
        "### Короткий вывод\n"
        f"- **Что это значит:** {band['conclusion']}.\n"
        f"- **Получилось уверенно:** {solved_text}.\n"
        f"- **Частично получилось:** {partial_text}.\n"
        f"- **Основные зоны роста:** {weak_text}.\n\n"
        "### План на ближайшие занятия\n"
        f"- **Первый фокус:** {band['focus']}.\n"
        f"- **Ориентир:** {band['target']}.\n"
        "- **Как будем двигаться:** на ближайших занятиях я разберу ошибки, закреплю слабые темы "
        "и буду фиксировать прогресс в личном кабинете после уроков и мини-срезов."
    )
